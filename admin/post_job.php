...
// Mode Edit: Jika ada ID di URL, ambil data lowongan yang ada
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $job_id = $_GET['id'];
    // Ambil data job, pastikan job tsb milik company yg login untuk keamanan
...
// Proses form (baik untuk Create maupun Update)
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        // Jika ada ID di hidden input, lakukan UPDATE. Jika tidak, lakukan INSERT.
        ...
        if ($stmt->execute()) {
            header("Location: " . BASE_URL . "/admin/jobs.php?status=success");
            exit();
        } else {
...
<body>
    ...
    <div class="container">
        <div class="form-container" style="max-width: 800px;">
            ...
            <form action="<?= BASE_URL ?>/admin/post_job.php<?= $job['id'] ? '?id=' . $job['id'] : '' ?>" method="POST">
